...
export const CustomerPicker: React.FC<CustomerPickerProps> = ({
  ...
  const {
    ...
    {
      // NOTE: Backend search currently behaves like "email-only" in some environments.
      // We fetch a reasonable base set and do client-side filtering by name/email/phone
      // so searching by customer name works reliably in the picker.
      search: "",
      limit: 200,
      offset: 0,
    },
    {
      skip: !state.isOpen && !preSelectedCustomerId && !selectedCustomerId,
    }
  );
  ...
  const allCustomers = useMemo(
    () =>
      (customersResponse || [])
        .filter(
          (customer) =>
            customer.type === "individual" || customer.type === "business"
        ) // Only business and individual customers
        .map((customer) => ({
          ...customer,
  ...
  const getCustomerColor = useCallback(
    (type: string) => {
      switch (type) {
        case "individual":
  ...
  useEffect(() => {
    if (preSelectedCustomerId && allCustomers.length > 0) {
      const preSelected = allCustomers.find(
  ...
  }, [selectedCustomerId, allCustomers, state.selectedCustomer?.id]);

  // Enhanced handlers with better state management
  const handleCustomerChange = useCallback(
    async (_event: any, newValue: EnhancedCustomerSummary | null) => {
      setState((prev) => ({
        ...prev,
      ...
      if (newValue && onCustomerDataChange) {
        try {
          ...
          const documents = await customerDocumentApi.getCustomerDocuments(
            newValue.id
          );
        ...
        const enhancedCustomer: EnhancedCustomerSummary = {
          ...
          name:
            newCustomer.type === "individual"
              ? `${(newCustomer as any).firstName} ${
                  (newCustomer as any).lastName
                }`.trim()
              : (newCustomer as any).legalName ||
                (newCustomer as any).administratorName,
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            ...
            helperText={
              error ||
              (isPreSelected
                ? "Customer pre-selected from account page"
                : state.hasInteracted
                ? "Search and select a business or individual customer"
        ...
        noOptionsText={
          <Box sx={{ textAlign: "center", py: 2 }}>
            <Typography variant="body2" color="text.secondary">
              {state.searchTerm
                ? "No business or individual customers found"
                : "Start typing to search business and individual customers"}
            </Typography>
          </Box>
        }
        loadingText={
          ...
          >
            ...
            <Typography variant="body2">
              Loading business and individual customers...
            </Typography>
          </Box>
        }
      />
        ...
        <Fade in timeout={300}>
          <Paper
            ...
            sx={{
              ...
              border: "2px solid",
              borderColor: alpha(
                getCustomerColor(state.selectedCustomer.type),
                0.3
              ),
              borderRadius: 2,
              bgcolor: alpha(
